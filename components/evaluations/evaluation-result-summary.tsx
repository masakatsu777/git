...
export function EvaluationResultSummary({ summary }: EvaluationResultSummaryProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        <article className="rounded-[1.75rem] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]"><p className="text-sm text-slate-500">参考本給額</p><p className="mt-3 text-2xl font-semibold text-slate-950">{formatCurrencyWithUnit(summary.gradeSalaryAmount)}</p></article>
        ...
        <article className="rounded-[1.75rem] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]"><p className="text-sm text-slate-500">自律成長給</p><p className="mt-3 text-2xl font-semibold text-slate-950">{formatCurrencyWithUnit(selfGrowthSalaryAmount)}</p></article>
        <article className="rounded-[1.75rem] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]"><p className="text-sm text-slate-500">協調相乗給</p><p className="mt-3 text-2xl font-semibold text-slate-950">{formatCurrencyWithUnit(synergySalaryAmount)}</p></article>
        ...
        <article className="rounded-[1.75rem] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)] md:col-span-2 xl:col-span-6"><p className="text-sm text-slate-500">現給差額</p><p className={`mt-3 text-2xl font-semibold ${referenceSalaryDiffAmount === 0 ? "text-slate-950" : referenceSalaryDiffAmount > 0 ? "text-emerald-700" : "text-rose-700"}`}>{formatSignedCurrencyWithUnit(referenceSalaryDiffAmount)}</p><p className="mt-1 text-sm text-slate-500">参考本給額 {formatCurrencyWithUnit(summary.gradeSalaryAmount)} - 現在本給 {formatCurrencyWithUnit(summary.currentSalary)}</p></article>
      </section>
    </div>
  );
}
