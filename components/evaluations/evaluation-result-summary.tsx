...
export function EvaluationResultSummary({ summary }: EvaluationResultSummaryProps) {
  ...
  const grossProfitAmountLabel = summary.grossProfitDeductionAmount < 0 ? "個人粗利の未達額" : "粗利差額配分";
  const grossProfitAmountDescription = summary.grossProfitDeductionAmount < 0
    ? "個人ベースで見た粗利未達分の目安金額です。"
    : "目標粗利との差額を配分する場合の目安金額です。";
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        <article className="rounded-[1.75rem] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]"><p className="text-sm text-slate-500">評価額</p><p className="mt-3 text-2xl font-semibold text-slate-950">{formatCurrencyWithUnit(summary.gradeSalaryAmount)}</p></article>
        ...
        <article className="rounded-[1.75rem] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]"><p className="text-sm text-slate-500">{grossProfitAmountLabel}</p><p className={`mt-3 text-2xl font-semibold ${summary.grossProfitDeductionAmount < 0 ? "text-rose-700" : summary.grossProfitDeductionAmount > 0 ? "text-emerald-700" : "text-slate-950"}`}>{formatSignedCurrencyWithUnit(summary.grossProfitDeductionAmount)}</p><p className="mt-1 text-sm text-slate-500">{grossProfitAmountDescription}</p></article>
