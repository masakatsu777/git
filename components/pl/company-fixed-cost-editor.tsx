...
export function CompanyFixedCostEditor({ canEdit, defaults }: CompanyFixedCostEditorProps) {
    ...
    <section className="rounded-[1.75rem] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">適用開始年月ごとに固定費を入力し、月次PLでは対象月以下の最新設定を社員人数比で按分します。</p>
        </div>
      ...
      <div className="mt-5 space-y-3">
        {rows.map((row) => (
          <div key={row.id} className="grid gap-3 rounded-2xl bg-slate-50 p-3 sm:grid-cols-[150px_1fr_180px_120px]">
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <button type="button" onClick={() => setRows((current) => [...current, { id: uid(), effectiveYearMonth: "", category: "", amount: 0 }])} disabled={!canEdit || isPending} className="rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700">
          行追加
        </button>
