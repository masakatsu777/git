...
export function UnassignedSalesEditor({ teamId, yearMonth, canEdit, employeeOptions, partnerOptions, defaults }: Props) {
  ...
  async function handleSave() {
    ...
    startSaving(async () => {
      ...
      setMessage(result.message ?? (response.ok ? "未所属入力を保存しました" : "未所属入力の保存に失敗しました"));
  ...
  return (
    <section className="rounded-3xl border border-stone-200 p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="font-semibold">未所属入力</h3>
          <p className="mt-1 text-xs text-stone-500">未所属の社員・パートナーの月次売上を入力します。admin と役員のみ更新できます。</p>
        </div>
      ...
      <p className="mt-3 text-sm text-stone-500">売上合計: {formatCurrencyWithUnit(salesTotal)}</p>
