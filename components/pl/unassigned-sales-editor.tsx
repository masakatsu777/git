...
export function UnassignedSalesEditor({ teamId, yearMonth, canEdit, employeeOptions, defaults }: Props) {
  ...
  async function handleSave() {
    ...
    startSaving(async () => {
      ...
      setMessage(result.message ?? (response.ok ? "未所属社員売上を保存しました" : "未所属社員売上の保存に失敗しました"));
  ...
  return (
    <section className="rounded-3xl border border-stone-200 p-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-semibold">未所属社員売上入力</h3>
          <p className="mt-1 text-xs text-stone-500">未所属社員の月次売上を入力します。admin と役員のみ更新できます。</p>
        </div>
        <button type="button" onClick={addAssignment} disabled={!canEdit || isPending || employeeOptions.length === 0} className="rounded-full border border-stone-300 px-3 py-1 text-xs font-medium">
          行追加
        </button>
      </div>
      ...
      <p className="mt-3 text-sm text-stone-500">売上合計: {salesTotal.toLocaleString("ja-JP")} 円</p>
