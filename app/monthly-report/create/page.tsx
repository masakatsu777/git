...
type MonthlyReportCreatePageProps = Readonly<{
...
export default async function MonthlyReportCreatePage({ searchParams }: MonthlyReportCreatePageProps) {
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#eef6ff_100%)] text-slate-900">
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mt-8">
          <MonthlyReportForm key={`${bundle.currentYearMonth}:${bundle.selectedProjectId}:${bundle.viewer.userId}:${bundle.source}`} initialBundle={bundle} />
        </div>
      </div>
    </main>
  );
