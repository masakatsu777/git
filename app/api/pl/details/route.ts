...
import { copyPreviousTeamMonthlyDetails, getPreviousYearMonth, getTeamMonthlyDetails, saveTeamMonthlyDetails } from "@/lib/pl/detail-service";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = action === "copyPrevious"
      ? await copyPreviousTeamMonthlyDetails(teamId, yearMonth)
      : await saveTeamMonthlyDetails({
          teamId,
          yearMonth,
    ...
    return NextResponse.json({
      message: action === "copyPrevious"
        ? (result.persisted
            ? `前月(${previousYearMonth})の明細をコピーし、月次PLを自動再計算しました`
            : `DB未接続のため前月(${previousYearMonth})コピーはプレビューのみです`)
        : (result.persisted ? "明細を保存し、月次PLを自動再計算しました" : "DB未接続のためプレビューのみ保持しました"),
